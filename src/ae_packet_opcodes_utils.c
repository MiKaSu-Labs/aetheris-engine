 ...
 * @author dkitagawa
 *
 * @file ae_packet_opcodes_utils.c
 *
 * @brief Auto-generated opcode-to-name lookup table.
 *
...
/* clang-format off */
static const ae_opcode_entry_t s_opcode_table[] = {
    { 0, "NONE" },
...
static const ae_size s_opcode_table_count =
    sizeof(s_opcode_table) / sizeof(s_opcode_table[0]);
...
{
    ae_size lo = 0;
    ae_size hi = s_opcode_table_count;

    if (opcode <= 0)
        return "UNKNOWN";

    while (lo < hi) {
        ae_size mid = lo + (hi - lo) / 2;
        int mid_val = s_opcode_table[mid].opcode;

        if (mid_val < opcode)
            lo = mid + 1;
        else if (mid_val > opcode)
            hi = mid;
        else
            return s_opcode_table[mid].name;
    }
...
static const int s_loop_packets[] = {
421,    /* WorldPlayerRTTNotify */
   704,    /* AvatarSatiationDataNotify */
   2319,   /* QueryPathRsp */
   2359,   /* QueryPathReq */
...
static const ae_size s_loop_packet_count =
    sizeof(s_loop_packets) / sizeof(s_loop_packets[0]);

ae_bool ae_is_loop_packet(int opcode)
{
    ae_size lo = 0;
    ae_size hi = s_loop_packet_count;
    ...
    while (lo < hi) {
        ae_size mid = lo + (hi - lo) / 2;
        int mid_val = s_loop_packets[mid];
