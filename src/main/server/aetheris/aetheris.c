 ...
 * @file aetheris.c
 *
 * @brief Aetheris server runtime: initialization, lifecycle management,
 * configuration, language loading, server startup, and console loop.
 *
...
/** Maximum number of bytes read from the console in a single line. */
#define AE_CONSOLE_INPUT_MAX 4096
...
/* Cast to int and compare against -1 to test whether this is set. */
...
/*
 * _signal_handler - SIGINT / SIGTERM handler.
 *
 * Calls ae_shutdown() then exits. The POSIX async-signal-safe subset is
 * limited; ae_shutdown() itself is not async-signal-safe, but this
 * pattern is the conventional approach for single-threaded teardown on
 * termination signals. Multi-threaded callers should replace this with a
 * write-to-pipe or atomic-flag approach if stricter safety is required.
 */
...
ae_error_t ae_main(int argc, char **argv) {
    ...
    default:
        ae_logger_error(ae_logger,
                        translate("messages.status.run_mode_error"), run_mode);
...
ae_error_t ae_load_config(void) {
    ...
    if (!ae_config) {
        ae_logger_error(ae_logger,
                        ...
                        "that there are no syntax errors. If you want to "
                        "start with a default configuration, delete your "
...
ae_error_t ae_save_config(const ae_config_t *config) {
    /*
     * When config is NULL, create a temporary default, write it, and free it
     * before returning. The temporary must be freed here because the caller
     * has no reference to it.
     */
    bool owns_config = (config == NULL);
    ae_config_t *to_save = owns_config
...
void ae_start_console(void) {
    if (ae_get_run_mode() == SERVER_RUN_MODE_DISPATCH_ONLY && ae_no_console) {
        ae_logger_info(ae_logger,
                       translate("messages.dispatch.no_commands_error"));
        return;
    }
    ...
    while (ae_config->server.game.enable_console) {
        ...
        if (rc == CONSOLE_INTERRUPTED) {
            ...
            ae_logger_error(ae_logger,
                            "An IO error occurred while trying to read "
                            "from console.");
            return;
        }
        ...
        if (command_map_invoke(ae_command_map, NULL, NULL, input) != AE_OK)
            ae_logger_error(ae_logger,
                            translate("messages.game.command_error"));
    }
}
...
void ae_update_day_of_week(void) {
    ...
    /* tm_wday is 0-based from Sunday; store as 1-based to match the
     * game data convention (1=Sunday, 7=Saturday). */
    ae_current_day_of_week = tm->tm_wday + 1;

    ae_logger_debug(ae_logger, "Set day of week to %d",
                    ae_current_day_of_week);
}
