...
ae_error_t ae_init(void) {
    ...
    if (!ae_logger) return AE_ERR_GENERIC;
    ...
    if (!ae_thread_pool) return AE_ERR_GENERIC;
...
ae_error_t ae_main(int argc, char **argv) {
    ...
    default:
        ...
        ae_logger_error(ae_logger,
            translate("messages.status.run_mode_help"));
        ae_logger_error(ae_logger,
            translate("messages.status.shutdown"));
...
ae_error_t ae_start_dispatch(void) {
    ...
    if (ae_get_run_mode() == SERVER_RUN_MODE_DISPATCH_ONLY) {
        ...
        if (!ae_dispatch_server) return AE_ERR_GENERIC;
...
ae_error_t ae_save_config(const ae_config_t *config) {
    const ae_config_t *to_save = config ? config : config_container_create_default();
...
void ae_start_console(void) {
    ...
    while (ae_config->server.game.enable_console) {
        ...
        if (rc == CONSOLE_INTERRUPTED) {
            ...
            ae_logger_error(ae_logger,
                "An IO error occurred while trying to read from console.");
            return;
        }
